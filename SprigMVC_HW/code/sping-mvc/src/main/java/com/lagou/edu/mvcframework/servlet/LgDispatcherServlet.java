...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            //类上的权限
                ...
                classSecurity = annotation.value();
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (method.isAnnotationPresent(Security.class)) {
                    ...
                    methodSecurity = securityMethod.value();
                }

                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(), method, Pattern.compile(url));
                handler.setClassSecurity(classSecurity);
    ...
    private void doScan(String scanPackage) {
        ...
        try {
            //路径有空格时需要转义
            scanPackagePath = java.net.URLDecoder.decode(scanPackagePath,"utf-8");
...
//        method.invoke() //
        ...
        if (handler.getMethodSecurity() != null &&
                !Arrays.stream(handler.getMethodSecurity()).anyMatch(x -> Arrays.asList(userNames).contains(x))) {
